...
public class SplayButton {
    public static void register(ButtonDispatcher<GuildButtonInteraction> dispatcher) {
        dispatcher.register("splay", interaction -> {
            ...
            if (audioChannel == null) {
                ...
                musicPlayer.load(data.url, new AudioLoadResultHandler() {
    ...
    public static Button create(String label, ButtonStyle style, Perms permission, String url) {
        return Button.of(style, new Gson().toJson(new Data("splay", permission, url)), label);
    }
