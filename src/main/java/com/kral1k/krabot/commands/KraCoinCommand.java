...
public class KraCoinCommand {
    public static void register(CommandDispatcher<GuildCommandInteraction> dispatcher) {
        dispatcher.register("kracoin", "kracoin").permission(source -> {
            ...
            interaction.replayTranslatable("bcoin.balance", member.getId(), member.getBalance()).setEphemeral(true).queue();
            ...
            interaction.replayTranslatable("bcoin.give", member.getId(), member.getBalance()).setEphemeral(true).queue();
            ...
            interaction.replayTranslatable("bcoin.set", member.getId(), member.getBalance()).setEphemeral(true).queue();
        }));
    }
}
