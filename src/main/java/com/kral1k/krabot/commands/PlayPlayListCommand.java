...
public class PlayPlayListCommand {
    public static void register(CommandDispatcher<GuildCommandInteraction> dispatcher) {
        dispatcher.register("plplay", "play play list").addOption(new OptionData(STRING, "url", "Название трека или url", true)).executor(interaction -> {
            ...
            if (name.startsWith("http://") || name.startsWith("https://") ) audioLoader.accept(name);
            else YouTubeSearch.newRequest(key, name).setType(YouTubeSearch.Type.PLAYLIST).setRegionCode("UA").sendAsync((statusCode, body) -> {
                ...
                else audioLoader.accept("https://www.youtube.com/playlist?list=" + body.getItem(0).getId().getPlaylistId());
            });
        });
    }
}
